'''
Create pickle files containing lookup tables to convert the instanes images to
NYU13. Needed because each trajectory in SceneNet has different indexes for objects.
...
if __name__ == '__main__':
    ...
    proto_dir = '/home/sean/hpc-cyphy/Datasets/scenenet_rgbd/data'
    lookup_dir = '/home/sean/hpc-cyphy/Datasets/scenenet_rgbd/data/' + \
        'objects_in_trajectories'
    protofiles = sorted([os.path.join(proto_dir, pp)
                         for pp in os.listdir(proto_dir) if pp.endswith('.pb')])
    assert os.path.isdir(
        lookup_dir), 'Invalid lookup_dir "{}"'.format(lookup_dir)
    nump = len(protofiles)
    for count, proto in enumerate(protofiles):
        print 'Loading {}/{} ({})'.format(count, nump, proto)
        ...
        for traj in trajectories.trajectories:
            ...
            s_proto = os.path.splitext(proto)[0].split('_')
            map_save = os.path.join(lookup_dir, s_proto[-2],
                                    str(traj.render_path))
            if not os.path.isdir(map_save):
